...
#define PREFIX "composite_types_as_variables"

TEST_CASE("function variable call that returns 50") {
    bloop::test::CheckConstant(MAKE_RELATIVE_PATH(PREFIX, "function_call"), Value::Type::t_int, 50);
}
TEST_CASE("array variable that returns [0, 1, 2]") {
    bloop::test::CheckArray(MAKE_RELATIVE_PATH(PREFIX, "array"), Value::Type::t_int, std::array{0,1,2});
}
TEST_CASE("object variable that returns { a: 1, b: 2, c: 3 }") {
